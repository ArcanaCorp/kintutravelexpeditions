...
export default function Page () {
    ...
    return (
        <main className="w-full">
            <section className="w-full flex flex-col gap-4 pv-4">
                <div className="relative w m-auto h bg-n1 rounded-md hidden" style={{"--w": "90%", "--h": "540px"}}>
                    <div className="absolute flex items-end w-full h-full bg-mask z p-4" style={{"--z": "2"}} data-aos="fade-up">
                        <h1 className="uppercase color-white fs-5xl lg:fs-6xl lg:w" style={{"--mxw-lg": "60%"}}>Vive las mejores experiencias</h1>
                    </div>
                    <picture className="inset-0 absolute block w-full h-full">
                        <source srcSet="./bg-experience.png" media="(min-width: 640px)" />
                        <source srcSet="./bg-experience.png" media="(min-width: 1024px)" />
                        <img src="./bg-experience.png" alt={config.HeroAlt} loading="lazy" decoding="async" referrerPolicy="no-referrer" draggable={false} />
                    </picture>
                </div>
            </section>
            <section className="w-full bg-n1 pv-8">
                <div className="w m-auto flex flex-col gap-8" style={{"--w": "90%"}}>
                    <h2 className="uppercase fs-4xl text-center" data-aos="fade-up">Más razones para experimentar</h2>
                    <div className="grid grid-3 gap-4">
                        {tours.map((tour) => (
                            <CardExperiences key={tour.id} exp={tour} size={'full'} />
                        ))}
                    </div>
                </div>
            </section>
            ...
            <section className="w-full pv-8 relative hidden">
